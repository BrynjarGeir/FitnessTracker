  ...
  <q-card class="bg-white full-width">
    <q-card-section class="bg-blue-grey-8">
      <div class="row items-center no-wrap">
        <div class="col">
          <div class="text-h6 text-white text-center">Total weight of press up exercises</div>
        </div>
      </div>
    </q-card-section>
    <q-card-section>
...
export default {
  ...
  methods: {
    async createChart (chartId) {
      ...
      const q = query(collection(db, 'fitnesstracker/' + userID + '/chest'), where('name', '==', 'Press Up'))
