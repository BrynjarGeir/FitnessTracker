  ...
  <q-card class="bg-white full-width">
    <q-card-section class="bg-blue-grey-8">
      <div class="row items-center no-wrap">
        <div class="col">
          <div class="text-h6 text-white text-center">Total iterations of inverse crunch exercises</div>
        </div>
      </div>
    </q-card-section>
    <q-card-section>
...
export default {
  ...
  methods: {
    async createChart (chartId) {
      ...
      const q = query(collection(db, 'fitnesstracker/' + userID + '/core'), where('name', '==', 'Inverse Crunch'))
      ...
      const myChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            data: totalI.slice(-8),//[86, 114, 106, 106, 107, 111, 133, 221, 783, 2478],
