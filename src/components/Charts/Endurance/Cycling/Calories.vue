  ...
  <q-card class="bg-white full-width">
    <q-card-section class="bg-blue-grey-8">
      <div class="row items-center no-wrap">
        <div class="col">
          <div class="text-h6 text-white text-center">Calories burned of cycling exercises</div>
        </div>
      </div>
    </q-card-section>
    <q-card-section>
...
export default {
  ...
  methods: {
    async createChart (chartId) {
      ...
      const q = query(collection(db, 'endurance'), where('name', '==', 'Cycling'))
      ...
      const myChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            data: calories,//[86, 114, 106, 106, 107, 111, 133, 221, 783, 2478],
            label: 'Calories',
            borderColor: '#3e95cd',
            fill: false,
            lineTension: 0.9
