  ...
  <q-card class="bg-white full-width">
    <q-card-section class="bg-blue-grey-8">
      <div class="row items-center no-wrap">
        <div class="col">
          <div class="text-h6 text-white text-center">Total weight of standing row exercises</div>
        </div>
      </div>
    </q-card-section>
    <q-card-section>
  ...
  methods: {
    async createChart (chartId) {
     ...
     const auth = getAuth()
      ...
      const q = query(collection(db, 'fitnesstracker/' + userID + '/back'), where('name', '==', 'Standing Row'))
