  ...
  <q-card class="bg-white full-width">
    <q-card-section class="bg-blue-grey-8">
      <div class="row items-center no-wrap">
        <div class="col">
          <div class="text-h6 text-white text-center">Total weight of seated cable row exercises</div>
        </div>
      </div>
    </q-card-section>
    <q-card-section>
...
export default {
  ...
  methods: {
    async createChart (chartId) {
      ...
      const q = query(collection(db, 'fitnesstracker/' + userID + '/back'), where('name', '==', 'Seated Cable Row'))
      ...
      const myChart = new Chart(ctx, {
        ...
        options: {
          ...
          title: {
            display: true,
            text: 'The total weight (accumulative) of each exercises (in kilogramms)'
          }
        }
      })
      return myChart
